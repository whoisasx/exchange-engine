...
EngineCheckpoint make_settled_funding_checkpoint(std::string checkpoint_id) {
  auto runtime = make_runtime();
...
void test_file_store_persists_and_restores_settled_funding_state() {
  TempDirectory temp;
  FileCheckpointStore store(temp.path);

  store.save(make_settled_funding_checkpoint("checkpoint-0005"));

  const auto loaded = store.load_latest();
  assert(loaded.has_value());
  assert(loaded->checkpoint_id == "checkpoint-0005");
  assert(loaded->source_position.next_offset == 1505);
  assert(loaded->public_sequences.at(1) == 13);
  ...
  const auto maker_key = PositionRiskKey{.user_id = 42, .market_id = 1};
  const auto taker_key = PositionRiskKey{.user_id = 43, .market_id = 1};
  assert(loaded->settled_funding_intervals.contains(settlement_key));
  assert(loaded->risk_states.at(maker_key).equity == 90);
  assert(loaded->risk_states.at(taker_key).equity == 110);
  assert(loaded->processed_input_ids.contains("input_funding_settle_001"));
  assert(loaded->processed_input_ids.at("input_funding_settle_001")
...
int main() {
  try {
    ...
    test_file_store_persists_and_restores_settled_funding_state();
